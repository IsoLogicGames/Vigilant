...
return function()
	...
	describe("Event", function()
		...
		it("should expose a registrar", function()
			...
			expect(event.Registrar.Connect).to.be.a("function")
			expect(event.Registrar.Connect).to.equal(event.Connect)
			expect(event.Registrar.Disconnect).to.be.a("function")
		...
		it("should expose an activator", function()
			...
			expect(event.Activator.Fire).to.be.a("function")
		...
		it("should allow callbacks to be disconnected", function()
			...
			expect(fired).to.equal(true)
			fired = false
			expect(function() event:Disconnect(callback) end).never.to.throw()
			expect(fired).to.equal(false)
			event:Fire()
			expect(fired).to.equal(false)
		end)
